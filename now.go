...
func (now *Now) Between(begin, end string) (bool, error) {
	beginTime, err := Parse(begin)
	if err != nil {
		return false, err
	}
	...
	if endTime.Before(beginTime) {
		return false, errors.New("wrong order of parameters")
	}

	return now.After(beginTime) && now.Before(endTime), nil
}

// MustBetween must check time between begin time and end time or not
func (now *Now) MustBetween(begin, end string) bool {
	flag, err := now.Between(begin, end)
	if err != nil {
		panic(err)
	}
